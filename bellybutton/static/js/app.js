...
function buildMetadata(sample) {
  ...
  d3.json(`/metadata/${sample}`).then((data) => {
    ...
    var SelPanel = d3.select("#sample-metadata");
    ...
    Object.entries(data).forEach(([key, value]) => {
      SelPanel.append("h6").text(`Key: ${key} and Value ${value}`)});
...
function buildCharts(sample) {
  ...
  d3.json(`/samples/${sample}`).then((data) => {
    ...
    //Build a Bubble Chart using the sample data
    // Build a Pie Chart using slice() to grab the top 10 sample_values, otu_ids, and labels (10 each).
    ...
    Plotly.plot("pie", Data_P, Layout_P);
